# -*- coding: utf-8 -*-
"""
Created on Mon Feb 14 16:02:11 2022
...
def split_voltages(V,dV):
    ...
    return Us
def teff(V,T,I,volume):
    return (T**5+V*I/(sigma_AL*volume))**(1/5)
class CBTmain: #just does the simulation, no further analysis
    ...
    def __init__(self,U,T,Ec,Gt,N,Nruns,Ntransient,number_of_concurrent,Ninterval,skip_transient,parallelization='external',
                 n0=None,second_order_C=None,dtype='float64',offset_C=None,dC=0,n_jobs=2,batchsize=1,q0=0,island_size=1.1544e-14,Ieff=None):
        """
        ...
        -------
        CBT object
            The raw result of the simulation to be inserted into a CBT_data_analysis instance to extract currents and conductances.
        ...
        self.q0=q0
        self.island_size=island_size*self.N
        if iterable(U):
        ...
        if Ieff is None:
            self.Ieff=self.Gt*np.array([self.U]).flatten()
        else:
            self.Ieff=Ieff
        ...
        if (self.parallelization=='internal'):
            if batchsize>1:
                ...
                self.dQps=[]
                self.dtps=[]
                for V_i in np.arange(len(voltbatch)):
                    self.U=voltbatch[V_i]
                    self.Ieff=currentbatch[V_i]
                    self.ueff=self.Ec/(kB*teff(self.U,self.T,self.Ieff,self.island_size))
                    self.number_of_Us=len(self.U)
            ...
            if batchsize==1:
                ...
                # print([S for S in list(zip(voltages,currents))])
                self.Is=Parallel(n_jobs=n_jobs,verbose=50)(delayed(f_scalar)(S) for S in list(zip(voltages,currents)))
            else:
                # print([S for S in list(zip(voltbatch,currentbatch))])
                self.Is=Parallel(n_jobs=n_jobs,verbose=50)(delayed(f_vector)(S) for S in list(zip(voltbatch,currentbatch)))
        try:
            self.U=voltages
    ...
    def update_number_of_concurrent(self,number_of_concurrent):
        ...
        if iterable(self.U)==False:
            ...
            self.ueff_vec=np.array(self.ueff).repeat(number_of_concurrent*2*self.N)
    ...
    def update_transition_rate(self,n1):
        ...
        if dE.ndim==1:
            ...
            Gamma=1/(self.ueff_vec*exprel(-self.ueff_vec*dE))#-dE/np.expm1(-dE*self.u)#(1-np.exp(-dE*self.u))
...
def carlo_CBT(U,T,Ec,Gt,N=100,Nruns=5000,Ntransient=5000,number_of_concurrent=5,Ninterval=1000,skip_transient=True,parallelization='external',
             ...
             make_plots=False,save_plots=True,output='full',transient=10,Ieff=None,island_size=1.1544e-14):
    """
    ...
    raw_result=CBTmain(Vs,T,Ec,Gt,N,Nruns=Nruns,
                       ...
                       n_jobs=n_jobs,
                       batchsize=batchsize,
                       q0=q0,island_size=island_size,Ieff=Ieff)
    if split_voltage:
